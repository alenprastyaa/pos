  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="openUploadModal"
            class="btn-secondary-emerald flex items-center gap-2 px-4 py-2 ed-lg transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <button @click="openCreateModal" class="btn-plain flex items-center gap-2 px-4 py-2 rounded-lg transition">
      ...
      <div class="flex gap-4">
        <div class="flex-1">
          <input ref="barcodeInputFocus" v-model="searchProduct" type="text" placeholder="Cari produk (nama/barcode)..."
            @keyup.enter="handleProductSearchEnter"
          ...
          <table class="min-w-full divide-y divide-border">
            <thead>
              <tr class="table-header border-b border-divider">
                ...
                <th class="px-5 py-3 text-left sm:px-6">
                  <p class="font-medium text-xs uppercase tracking-wider th-text">
                    Harga Jual Ritel
                  </p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6">
            ...
            <tbody class="divide-y divide-border bg-white">
              ...
              <tr v-for="(produk, index) in productList" :key="produk.id" class="border-t border-divider row-hover">
      ...
      <div v-if="showModal" class="fixed inset-0 z-[100000] flex items-center justify-center modal-overlay">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 relative">
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            <div>
              ...
              <input ref="barcodeInput" v-model="formData.barcode" type="text" placeholder="Masukkan barcode" required
                :disabled="isEditing" class="input-field h-11 w-full rounded-lg px-4 py-2.5 text-sm" />
            </div>
            ...
            <div>
              ...
              <input v-model="formData.nama_produk" type="text" placeholder="Masukkan nama produk" required
            ...
            <div>
              ...
              <input v-model.number="formData.stok_produk" type="number" min="0" placeholder="Stok awal" required
                class="input-field h-11 w-full rounded-lg px-4 py-2.5 text-sm" />
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input type="text" placeholder="Harga Jual Ritel" required :value="formData.harga_jual_ritel > 0 ? formatToRupiah(formData.harga_jual_ritel) : ''
              ...
              <div>
                ...
                <input type="text" placeholder="Harga Jual Biasa" required :value="formData.harga_jual_biasa > 0 ? formatToRupiah(formData.harga_jual_biasa) : ''
            ...
            <div class="flex gap-3 pt-4 border-t border-divider mt-4">
              <button type="button" @click="closeModal" class="btn-cancel flex-1 px-4 py-2 rounded-lg transition">
                Batal
              </button>
              <button type="submit" :disabled="isSubmitting"
                class="flex-1 px-4 py-2 rounded-lg transition disabled:opacity-50 text-white bg-blue-500">
      ...
      <div v-if="showUploadModal"
        class="fixed inset-0 z-[100000] flex items-center justify-center modal-overlay backdrop-blur-sm">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative">
          <div class="flex items-center justify-between mb-4 border-b border-divider pb-3">
            <h2 class="text-xl font-semibold text-title">Upload Produk via Excel</h2>
          ...
          <form @submit.prevent="submitUploadExcel" class="space-y-4">
            ...
            <div v-else class="pt-2">
              <p class="text-sm text-muted">
                Produk akan diupload ke toko Anda:
                <span class="font-semibold text-title">{{
                  userData?.toko.nama_toko || 'Memuat...'
                }}</span>
              </p>
            </div>

            <div class="flex gap-3 pt-4 border-t border-divider mt-4">
              <button type="button" @click="closeUploadModal" class="btn-cancel flex-1 px-4 py-2 rounded-lg transition">
                Batal
              </button>
...
const lastAutoOpenedBarcode = ref('')
watch(
  () => showModal.value,
  (newVal) => {
...
const fetchProducts = async () => {
  ...
  try {
    ...
    if (response.data.success && response.data.data) {
      ...
      openEditModalIfExactBarcodeMatch()
    } else {
      productList.value = []
      pagination.value = {
        total: 0,
...
const normalizeBarcode = (value: string | number | null | undefined) => {
  return String(value ?? '').trim()
}

const openEditModalIfExactBarcodeMatch = () => {
  const scannedBarcode = normalizeBarcode(searchQuery.value)
  if (!scannedBarcode || showModal.value) return

  const matchedProducts = productList.value.filter((product) => {
    return normalizeBarcode(product.barcode) === scannedBarcode
  })

  if (matchedProducts.length !== 1) return
  if (lastAutoOpenedBarcode.value === scannedBarcode) return

  lastAutoOpenedBarcode.value = scannedBarcode
  openEditModal(matchedProducts[0])
}
...
const downloadProductsExcel = async () => {
  ...
  try {
    ...
    const fallbackName = isSuperAdmin.value
      ? `produk_semua_toko_${new Date().toISOString().slice(0, 10)}.xlsx`
      : `produk_${new Date().toISOString().slice(0, 10)}.xlsx`
    ...
    Swal.fire(
      'Error',
      error.response?.data?.message || 'Gagal mendownload file Excel produk.',
      'error',
    )
  } finally {
